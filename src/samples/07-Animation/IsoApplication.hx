...
#else
		xTween = cast Actuate.update(box.set_x, 2, [box.get_x()], [pt.x]).onComplete(tweenCompleteHandler);
		yTween = cast Actuate.update(box.set_y, 2, [box.get_y()], [pt.y]).onComplete(tweenCompleteHandler);
#end
