...
class ApplicationMain
{
	...
	{
		#if openfl
		flash.Lib.setPackage("haxeports", "Sample", "com.googlecode.haxeports", "1.0.0");
		...
		wx.App.boot(function()
		{
			...
			frame = wx.Frame.create(null, null, "as3isolib sample", null, { width: 640, height: 480 });
			...
			var hasMain = false;
			for (methodName in Type.getClassFields(IsoApplication)) {
				if (methodName == "main") {
					hasMain = true;
					break;
				}
			...
			if (hasMain) {
				Reflect.callMethod(IsoApplication, Reflect.field(IsoApplication, "main"), []);
			}else {
				var instance = Type.createInstance(IsoApplication, []);
			}
			
			/*if (Reflect.hasField(IsoApplication, "main")) {
				IsoApplication.main();			
			}*/
...
class DocumentClass extends IsoApplication {}
